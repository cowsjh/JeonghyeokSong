...
내부적으로 노드는 인풋으로 받은 지오메트리를 에셋에서 사용 가능하게 재구성 하기 때문에 외부에서 자유롭게 지오메트리를 조작 해도 오류없이 출력 할 수 있다. 이를 이용 하여 원하는 쉐잎을 보다 쉽게 얻을 수 있다.
![alt text](image-1.png)
...
가지를 카드로 만들고 인스턴싱하는 방법은 원하는 모양을 찾기 힘들 수 있기 때문에 다른 워크플로우를 추가적으로 고안해 내었는데, 하이 폴리로 만들어진 나무에서 원하는 레벨의 특정한 가지를 카드로 선별하는 기능이다. 해당 기능은 파라미터로 가지를 선별하고 원하는 방향에서 프로젝션하여 카드로 만들 수 있다. 이 과정에서 x, y, z 평면을 참조하는 방법을 사용했지만 **좀 더 직관적인 조작이 필요하다고 느껴 파이썬과 VEX를 통해 카메라 프로젝션을 도입했다.** 확실히 원하는 모양의 레퍼런스를 쉽고 빠르게 찾을 수 있었다.
...
# Result
노드 구조에 대한 테스트. 노드 특성상 재활용이 가능 하다는 큰 장점이있는데, 원하는 잔가지 + 잎 들의 디자인을 제작 하였다면 일관성을 가지고 다른 가지에도 적용 시킬 수 있다.
![](jh-render02-0001.png)
Branch 노드를 통한 뿌리 디자인과 카드 인스턴싱의 반복 패턴에 대한 테스트. 뿌리 자체는 가지와 같은 메커니즘으로 해석 하여 따로 노드는 제작 하지않았고 대신 Branch 노드에서 포괄적으로 다룰 수 있게 디자인 했다.
![](jh-render02-0002.png)
일정한 층을 가지고 있는 듯한 소나무를 통해 Branch 노드의 패턴을 테스트 해보았다. 이런 패턴은 노이즈나 랜덤성으로만 구현하기에는 무리가 있고 규칙을 가진 파라미터 로 표현이 가능하다.
![](jh-render02-0003.png)
가지와 잎의 클러스터링 패턴 테스트. 필터링 파라미터를 이용하여 뭉쳐 보이는 패턴을 구현 하여 자연스러운 연출이 가능하게 디자인 했다.
...
![alt text](image-2.png)
## Workflow
 Module 형식으로 제작한 이유는 사용자가 노드 중간에서 지오메트리를 조작할 수 있도록 하기 위해서다. 또한 하이 메쉬로 제작 되기 때문에 순차적으로 빌드하는 것이 효율이 좋을 거라고 생각했다. 
![alt text](image-3.png)
...
### Pattern
대부분의 패턴은 VOP 안에서의 attribute와 noise 를 조합 하여 제작 되었다.
![alt text](<2026-04-28 18-27-55_trimmed.gif>)

### Lowpoly
highpoly 와 lowpoly가 병렬적으로 생성 되는 워크플로우를 채택했다. 각 노드의 연산 비용이 증가하지만 UV 보존을 용이하게 하고 내가 원하는 디테일을 의도적으로 보존할 수 있다는 확실한 장점이 있다.
